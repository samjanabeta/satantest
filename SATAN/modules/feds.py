...
from SATAN.modules.helper_funcs.extraction import (
    extract_user,
    extract_unt_fedban,
    extract_user_fban,
)
from SATAN.modules.helper_funcs.string_handling import markdown_parser
from SATAN.modules.disable import DisableAbleCommandHandler

import SATAN.modules.sql.feds_sql as sql

from SATAN.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
...
def new_fed(update, context):
    ...
    if len(fednam) >= 2:
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact my owner @IMPERIALELITEX if the problem persists."
            )
            return
...
def leave_fed(update, context):
    ...
    if getuser in "creator" or user.id in SUDO_USERS:
        if sql.chat_leave_fed(chat.id) == True:
            ...
            update.effective_message.reply_text(
                "How can you leave a federation that you never joined? Lol!!"
            )
    else:
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "SATAN_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="SATAN_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "SATAN_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="SATAN_fbanned_users.csv",
