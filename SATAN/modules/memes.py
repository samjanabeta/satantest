...
from SATAN import dispatcher, SUDO_USERS, SUPPORT_USERS, LOGGER
from SATAN.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import SATAN.modules.helper_funcs.fun_strings as fun
...
def stretch(update, context):
    message = update.effective_message
    if not message.reply_to_message:
        message.reply_text("I need a message to streeetch.")
    else:
...
`/decide` can be also used with regex like: `SATAN? <question>: randomly answer "Yes, No" etc.`
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^SATAN\?"), decide, friendly="decide"
)
